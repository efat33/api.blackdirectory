...
class ReplyModel {
  ...
  getReplyNotifications = async (topic_id) => {
    const sql = `SELECT ReplyNotification.*, Topics.title as topic_title, Topics.slug as topic_slug,
    Forums.title as forum_title, Forums.slug as forum_slug,
    Users.display_name as user_display_name  
    FROM ${DBTables.reply_notifications} as ReplyNotification
    LEFT JOIN ${DBTables.users} Users ON Users.id = ReplyNotification.user_id 
    LEFT JOIN ${DBTables.topics} Topics ON Topics.id = ReplyNotification.topic_id 
    LEFT JOIN ${DBTables.forums} Forums ON Forums.id = Topics.forum_id 
    WHERE topic_id=?`;

    return await query(sql, [topic_id]);
  }
