...
const { isEmployer, isAdmin, isEmployerOrAdmin } = require('../utils/common');
...
router.post('/new-job', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.newJob));
router.put('/update-job/:job_id', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.updateJob));
router.delete('/delete-job/:job_id', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.deleteJob));

router.get('/get-user-jobs', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.getUserJobs));
router.get('/get-user-job/:job_id', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.getUserJob));
...
router.put('/update-job-property/:job_id', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.updateJobProperty));
...
router.get('/get-job-packages', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.getJobPackages));
router.get('/get-current-package', apiKey(), auth(), isEmployerOrAdmin(), awaitHandlerFactory(jobController.getCurrentPackage));
